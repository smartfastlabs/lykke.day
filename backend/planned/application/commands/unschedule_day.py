...
class UnscheduleDayHandler(CommandHandler[UnscheduleDayCommand, objects.Day]):
    """Handles UnscheduleDayCommand."""

    def __init__(self, uow_factory: UnitOfWorkFactory) -> None:
        self._uow_factory = uow_factory

    async def handle(self, cmd: UnscheduleDayCommand) -> objects.Day:
        ...
        async with self._uow_factory.create(cmd.user_id) as uow:
            ...
            day_id = objects.Day.id_from_date_and_user(cmd.date, cmd.user_id)
            try:
