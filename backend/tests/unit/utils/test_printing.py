...
async def test_generate_pdf_from_page():
    ...
    with patch("lykke.core.utils.printing.async_playwright") as mock_playwright:
        mock_p = MagicMock()
        ...
        with patch("lykke.core.utils.printing.tempfile.NamedTemporaryFile") as mock_tmp:
...
async def test_send_pdf_to_printer():
    ...
    with patch("lykke.core.utils.printing.create_subprocess_exec") as mock_exec:
        mock_exec.return_value = mock_proc

        with patch("lykke.core.utils.printing.aiofiles.os.remove") as mock_remove:
