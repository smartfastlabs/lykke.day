...
class TestEntityTypeNamingConsistency:
    ...
    def test_day_entity_type_matches_filter(self):
        ...
        causing goal updates to not be pushed via websocket.
        """
...
class TestIsAuditLogForTodayTaskFiltering:
    ...
    async def test_task_for_today_returns_true(self):
        ...
        result = await is_audit_log_for_today(audit_log, today)
...
class TestIsAuditLogForTodayDayFiltering:
    ...
    async def test_day_for_today_returns_true(self):
        ...
        for all day updates, which caused goal changes to not be pushed.
        """
        today = dt_date(2025, 1, 15)
        audit_log = _make_audit_log(
            entity_type="day",
    ...
    @pytest.mark.asyncio
    async def test_day_with_goals_for_today_returns_true(self):
        """Test day entity with goals for today returns True.

        This simulates a goal being added/updated/removed.
        """
        today = dt_date(2025, 1, 15)
        audit_log = _make_audit_log(
            entity_type="day",
            entity_data={
                "date": "2025-01-15",
                "goals": [{"id": str(uuid4()), "name": "Test Goal", "status": "INCOMPLETE"}],
            },
        )
