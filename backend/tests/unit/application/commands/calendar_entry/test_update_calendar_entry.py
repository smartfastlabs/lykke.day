...
@pytest.mark.asyncio
async def test_cannot_mark_missed_before_meeting_starts() -> None:
    user_id = uuid4()
    calendar_id = uuid4()
    entry = CalendarEntryEntity(
        user_id=user_id,
    ...
    update_object = CalendarEntryUpdateObject(
        attendance_status=CalendarEntryAttendanceStatus.MISSED
    )
...
@pytest.mark.asyncio
async def test_cannot_mark_not_going_after_meeting_starts() -> None:
    user_id = uuid4()
    calendar_id = uuid4()
    entry = CalendarEntryEntity(
        user_id=user_id,
    ...
    update_object = CalendarEntryUpdateObject(
        attendance_status=CalendarEntryAttendanceStatus.NOT_GOING
    )
