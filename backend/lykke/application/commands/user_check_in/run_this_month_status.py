...
class ThisMonthsStatusHandler(
    ...
    async def handle(self, command: ThisMonthsStatusCommand) -> None:
        ...
        if result is None:
            return
        logger.debug(
            f"this_month_status completed for user {self.user.id}, "
    ...
    def build_tools(
        ...
        *,
        ...
        """Single tool: record_months_status(text=None, scores=None)."""

        async def record_months_status(
            text: str | None = None,
            scores: dict[str, float] | None = None,
        ) -> None:
            """Record this month's status check-in from the LLM."""
            scores = scores or {}
            ...
            logger.info(
                f"Persisted this_month_status check-in for user {self.user.id}"
            )

        return [LLMTool(callback=record_months_status)]
