...
class BotPersonality(Base):
    ...
    visibility = Column(String, nullable=False)  # BotPersonalityVisibility enum as string
    rating = Column(Float, nullable=False, default=0.0)  # For marketplace sorting
    usage_count = Column(Integer, nullable=False, default=0)  # Popularity metric
    meta = Column(JSONB)  # Additional config
    created_at = Column(DateTime, nullable=False)

    __table_args__ = (
        Index("idx_bot_personalities_user_id", "user_id"),
        Index("idx_bot_personalities_visibility", "visibility"),
        Index("idx_bot_personalities_rating", "rating"),
