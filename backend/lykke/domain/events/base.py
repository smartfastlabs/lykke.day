...
class AuditedEvent:
    """Mixin class for domain events that should generate audit logs.

    Events inheriting from this class will automatically create AuditLog
    entries when processed by the Unit of Work. The default implementation
    extracts all event fields into the audit log's meta field and infers
    entity_id and entity_type from the event's fields.
    ...
    Example:
        ...
        # - activity_type: "MyEvent"
        # - entity_id: task_id
        # - entity_type: "task" (inferred from "task_id")
        # - meta: {"task_id": "...", "some_field": "..."}
    """
    ...
    def to_audit_log(self, user_id: UUID) -> "AuditLogEntity":
        ...
        for field_name, field_value in event_dict.items():
            if field_name.endswith("_id") and field_name != "user_id" and isinstance(field_value, UUID):
