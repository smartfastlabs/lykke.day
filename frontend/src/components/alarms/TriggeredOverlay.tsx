...
const AlarmTriggeredOverlay: Component<AlarmTriggeredOverlayProps> = (
  ...
  return (
    <ModalOverlay
      ...
      overlayClass="fixed inset-0 z-[70] flex items-center justify-center"
      backdropClass="absolute inset-0 bg-stone-900/60 backdrop-blur-[2px]"
    ...
    >
      <div class="text-center space-y-2">
        ...
        <Show when={props.alarm?.name}>
          <p class="text-lg font-semibold text-white">{props.alarm?.name}</p>
        </Show>
        <Show when={!props.alarm?.name}>
