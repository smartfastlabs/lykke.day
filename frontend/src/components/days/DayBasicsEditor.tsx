...
export type DayBasicsEditorProps = {
  day: () => Day | undefined;
  label: string;
  backHref: string;
...
export const DayBasicsEditor: Component<DayBasicsEditorProps> = (props) => {
  ...
  createEffect(() => {
    const currentDate = props.day()?.date ?? null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Failed to update day";
      setError(message);
    } finally {
      setIsSaving(false);
    }
    ...
    <div class="max-w-2xl">
      <div class="rounded-2xl border border-amber-100/80 bg-white/70 shadow-lg shadow-amber-900/5 backdrop-blur-sm p-4 space-y-5">
        <div>
          ...
          <h2 class="text-lg font-medium text-stone-900">Update your day basics</h2>
...
export default DayBasicsEditor;
